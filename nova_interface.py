...
def display_banner():
    ...
    banner_text.append("/_/|_/ \\___/|___/ \\_,_/ /_/  /_//_//_//_/\\_,_/  \n", style="bold magenta")
...
def main():
    ...
    with console.status("[bold magenta]Igniting NovaMind...[/]", spinner="dots"):
        try:
            ...
            router = NovaToolRouter(chat_engine.model, chat_engine.tokenizer, chat_engine.model.config)
    ...
    while True:
        try:
            ...
            with Live(Spinner("bouncingBar", text="[magenta]Processing...[/]"), refresh_per_second=10) as live:
                # 1. Routing
            ...
            console.print(
                f"[dim]Stats: {tokens} tokens | {elapsed:.2f}s response time[/dim]\n"
            )
...
def _display_tool_output(intent: str, output: dict, context_result: str = None):
    ...
    if intent == "code":
        ...
        if output.get("success"):
            ...
            console.print(Panel(output.get("error", "Error"), title="[error]Code Error[/]", border_style="red"))
            
    elif intent == "search":
        # Show sources from context
        if context_result:
            console.print(Panel(context_result, title=f"[bold yellow]{title}[/]", border_style="yellow"))
            
    elif intent == "file":
        if "content" in output:
             syntax = Syntax(output["content"][:1000], "python", theme="monokai", line_numbers=True)
             console.print(Panel(syntax, title=f"File: {output.get('path', 'unknown')}", border_style="blue"))
        else:
             console.print(Panel(str(context_result), title="File System Response", border_style="blue"))
