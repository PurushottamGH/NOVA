...
def verify():
    ...
    config = NovaMindConfig(
        vocab_size=100, embed_dim=128, num_heads=4, num_layers=2, context_length=32
    )
    ...
    with torch.inference_mode():
        _logits_1, past_kv = model(input_ids, use_cache=True)
        # Pass 2: Use last token + cache
        torch.randint(0, 100, (1, 1))  # placeholder for now,
