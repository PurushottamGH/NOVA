...
def compute_perplexity(loss: torch.Tensor) -> float:  # FIXED: added perplexity helper — easier to interpret than raw loss
    """
...
class LabelSmoothingCrossEntropy(nn.Module):
    ...
    def __init__(self, smoothing: float = 0.05, ignore_index: int = 0):  # FIXED: default smoothing=0.05 to match trainer usage
        super().__init__()
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            smooth_targets = torch.full_like(logits, self.smoothing / (vocab_size - 1))  # FIXED: was / vocab_size
            # Set the true class to (1 - smoothing)
            smooth_targets.scatter_(1, targets.unsqueeze(1), 1.0 - self.smoothing)  # FIXED: simplified — no need to add back smoothing/vocab
