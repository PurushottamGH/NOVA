    ...
    def __getitem__(self, idx):
        ...
        if len(input_ids) > self.context_length:
            ...
            target_ids = torch.cat(
                [target_ids, torch.full((padding_len,), PAD_TOKEN_ID, dtype=torch.long)]
            )
...
def sft_train(model_path, tokenizer_path, data_dir, epochs=3, lr=1e-4, batch_size=4, device="auto"):
    ...
    for epoch in range(epochs):
        ...
        for _batch_idx, (input_ids, target_ids) in enumerate(dataloader):
            ...
            if global_step % 50 == 0:
                print(
                    f"  Step {global_step} | Loss: {loss.item():.4f} | LR: {optimizer.param_groups[0]['lr']:.2e}"
                )
        ...
        print(
            f"  ✓ Epoch {epoch + 1} complete in {time.time() - epoch_start:.0f}s. Checkpoint saved."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path", type=str, default="weights/final_model", help="Path to base model"
    )
    parser.add_argument(
        "--tokenizer_path", type=str, default="weights/tokenizer", help="Path to tokenizer"
    )
    parser.add_argument(
        "--data_dir", type=str, default="personal_data", help="Directory with .txt SFT data"
    )
