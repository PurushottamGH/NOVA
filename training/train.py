...
def main():
    ...
    parser.add_argument(
        "--data_dir", type=str, default="personal_data", help="Directory with training .txt files"
    )
    ...
    parser.add_argument(
        "--checkpoint_dir", type=str, default="weights", help="Checkpoint directory"
    )
    parser.add_argument(
        "--tokenizer_dir", type=str, default="tokenizer_data", help="Tokenizer save directory"
    )
    ...
    if checkpoint_file.exists() and not args.new:
        ...
        print("  [Fresh Start] '--new' flag detected — starting from step 0")
    else:
    ...
    )  # FIXED: use Path for Windows compat
