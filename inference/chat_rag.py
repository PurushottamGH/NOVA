...
def main():
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="weights", help="Model weights directory")
    parser.add_argument("--data_dir", type=str, default="personal_data", help="Directory with documents to index")
    ...
    config = NovaMindConfig.from_dict(torch.load(f"{args.checkpoint_dir}/latest.pt", weights_only=False)["config"])
    if args.device != "auto":
        config.device = args.device
    
    model = NovaMind(config)
    # Load weights
    checkpoint = torch.load(f"{args.checkpoint_dir}/latest.pt", map_location=config.device, weights_only=False)
