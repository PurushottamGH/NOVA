...
class MultiHeadCausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.W_qkv = nn.Linear(self.embed_dim, 3 * self.embed_dim, bias=False)  # FIXED: was 3 separate Linear layers
        ...
        causal_mask = torch.tril(
            torch.ones(config.context_length, config.context_length, dtype=torch.bool)  # FIXED: was float, now bool
        ).unsqueeze(0).unsqueeze(0)  # (1, 1, context_length, context_length)
        self.register_buffer("causal_mask", causal_mask)
    
    def forward(self, x: torch.Tensor, past_kv=None, use_cache=False):  # FIXED: added past_kv and use_cache for KV caching
        """
        ...
        """
        batch_size, seq_len, embed_dim = x.shape  # (batch, seq_len, embed_dim)
        ...
        if past_kv is not None:
            ...
            K = torch.cat([past_K, K], dim=2)  # (batch, num_heads, past_seq_len + seq_len, head_dim)
            V = torch.cat([past_V, V], dim=2)  # (batch, num_heads, past_seq_len + seq_len, head_dim)
        ...
        attention_scores = torch.matmul(Q, K.transpose(-2, -1)) / scale  # (batch, num_heads, seq_len, kv_seq_len)
        ...
        # ============================================================
        attention_scores = attention_scores - attention_scores.amax(dim=-1, keepdim=True)  # FIXED: numerical stability — prevents exp() overflow
        
        attention_weights = F.softmax(attention_scores, dim=-1)  # (batch, num_heads, seq_len, kv_seq_len)
        
        attention_weights = torch.nan_to_num(attention_weights, nan=0.0)  # FIXED: guard against any remaining NaN after softmax
        
        # Apply dropout to attention weights (regularization during training)
        attention_weights = self.attn_dropout(attention_weights)  # (batch, num_heads, seq_len, kv_seq_len)
        ...
        # ============================================================
        attended = attended.transpose(1, 2).contiguous().view(batch_size, seq_len, self.embed_dim)  # (batch, seq_len, embed_dim)
